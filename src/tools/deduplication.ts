...
type ScanResourceType = (typeof SCAN_RESOURCE_TYPES)[number];
const SCAN_RESOURCE_SET = new Set<string>(SCAN_RESOURCE_TYPES);
...
function arrayData(doc: JsonApiDocument<JsonApiResource[] | JsonApiResource>): JsonApiResource[] {
...
function resourceDisplayName(r: JsonApiResource): string {
  const a = (r.attributes ?? {}) as Record<string, unknown>;
  const direct =
    typeof a.name === "string" ? a.name : typeof a.title === "string" ? a.title : "";
...
export const deduplicationTools: ToolDefinition[] = [
  {
    ...
    handler: async (args, { client }) => {
      ...
      const matches = candidates
        .map((r) => {
          ...
          const serialExact =
            normSerial.length > 0 && cSerial.length > 0 && normalize(cSerial) === normSerial;
          const assetExact =
            normAsset.length > 0 && cAsset.length > 0 && normalize(cAsset) === normAsset;
  ...
  },
  {
    ...
    handler: async (args, { client }) => {
      ...
      if (!SCAN_RESOURCE_SET.has(resourceType)) {
        throw new Error(
          `Unsupported resourceType. Must be one of: ${SCAN_RESOURCE_TYPES.join(", ")}.`,
        );
      }
      const typedResource = resourceType as ScanResourceType;
      const threshold =
        typeof args.threshold === "number" ? args.threshold : DEFAULT_SCAN_THRESHOLD;
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        for (const r of data) {
          ...
          if (name) records.push({ id: r.id ?? "", name });
          if (records.length >= HARD_SCAN_CAP) {
            truncated = true;
            break;
          }
        }
        if (truncated || data.length < pageSize) break;
      }

      const grouped = new Set<number>();
      const groups: Array<Array<{ id: string; name: string; score: number }>> = [];
      for (let i = 0; i < records.length; i++) {
        if (grouped.has(i)) continue;
        const group: Array<{ id: string; name: string; score: number }> = [

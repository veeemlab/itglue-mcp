...
const ATTACHMENT_PARENT_SET = new Set(ATTACHMENT_PARENT_TYPES);
function assertParentType(value) {
    if (typeof value !== "string" || !ATTACHMENT_PARENT_SET.has(value)) {
        throw new Error(`Unsupported parentType. Must be one of: ${ATTACHMENT_PARENT_TYPES.join(", ")}.`);
    }
    return value;
}
export const attachmentTools = [
    {
        ...
        description: "List attachments on a parent resource. Returns metadata only — file names, sizes, content types, and download URLs (use the URL out-of-band to fetch the actual file).",
        inputSchema: {
            type: "object",
            properties: {
                parentType: {
        ...
        handler: async (args, { client }) => {
            ...
            return client.get(`/${parentType}/${encodeURIComponent(parentId)}/relationships/attachments`, query);
        },
    },
    {
        ...
        handler: async (args, { client }) => {
            ...
            return client.delete(`/${parentType}/${encodeURIComponent(parentId)}/relationships/attachments/${encodeURIComponent(id)}`);
        },
    },
];
